...
function Table({
  ...
  return (
    <div className={cx('overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm', className)} {...props}>
      <div className="overflow-x-auto">
        <table className="min-w-full border-separate border-spacing-0">
          <thead>
            <tr className="bg-slate-50 text-left text-xs font-black uppercase tracking-[0.18em] text-slate-500">
              {columns.map((col) => (
                <th key={col.key} scope="col" className={cx('px-4 py-3', col.className)}>
                  {col.header}
                </th>
              ))}
            </tr>
          ...
          <tbody className="text-sm font-semibold text-slate-700">
            {rows.length === 0 ? (
              <tr>
                    <td className="px-4 py-8 text-center text-sm font-bold text-slate-500" colSpan={columns.length || 1}>
                  {emptyState}
                </td>
              </tr>
            ) : (
